...
class CoCa(nn.Module):
    ...
    def _encode_image(self, images=None, normalize=True):
        image_output = self.visual(images)
        if isinstance(image_output, tuple):
            image_latent, tokens_embs = image_output
        else:
            image_latent = image_output
            tokens_embs = image_output.unsqueeze(1)
        image_latent = F.normalize(image_latent, dim=-1) if normalize else image_latent
        return image_latent, tokens_embs

    def _encode_text(self, text, normalize=True, embed_cls=True):
        # Only reserve the final position when this text tower has a CLS token.
        embed_cls = embed_cls and self.text.cls_emb is not None
        text = text[:, :-1] if embed_cls else text # make space for CLS token
        text_output = self.text(text)
        if isinstance(text_output, tuple):
            text_latent, token_emb = text_output
        else:
            text_latent = text_output
            token_emb = text_output.unsqueeze(1)
