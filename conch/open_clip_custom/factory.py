...
def create_model(
    ...
    if checkpoint_path is not None and checkpoint_path.startswith("hf_hub:"):
        repo_id = checkpoint_path[len("hf_hub:"):]
        _ = hf_hub_download(repo_id=repo_id,
                            cache_dir=cache_dir, filename="meta.yaml",
                            token=hf_auth_token)
        checkpoint_path = hf_hub_download(repo_id=repo_id,
    ...
    # `jit` is retained as a compatibility flag; it now requests an ExportedProgram.
    if jit:
        if example_inputs is None:
            raise ValueError("example_inputs=(image_tensor, text_tensor) is required when jit=True")
        model = torch.export.export(model, args=example_inputs)
...
def create_model_from_pretrained(
    ...
    if not return_transform and not jit:
        return model
